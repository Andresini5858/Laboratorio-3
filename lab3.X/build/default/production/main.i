...
; CONFIG1
  ...
  CONFIG BOREN = OFF ; Brown Out Reset Selection bits (BOR controlled by ((PCON) and 07Fh), 4 bit of the PCON register)
...
main:
    ...
    BANKSEL TRISD
    BSF TRISD, 6 ;((PORTD) and 07Fh), 6 COMO ENTRADA
    BSF TRISD, 7 ;((PORTA) and 07Fh), 7 COMO ENTRADA
...
loop:
    ...
    BTFSC PORTD, 7 ;Se verifica si el botón de ((PORTD) and 07Fh), 7 se apachó
    call anti1 ;Se llama al antirebote
    BTFSS PORTD, 7 ;Si el botón se dejo de presionar se llama a la función incrementar
    call incrementar1
    BTFSC PORTD, 6 ;Se verifica si el botón de ((PORTD) and 07Fh), 6 se apachó
...
veri:
    ...
    BTFSS STATUS, 2 ;Se chequea el bit ((STATUS) and 07Fh), 2, si el bit ((STATUS) and 07Fh), 2 es 1 la resta es 0 y se prosigue; si el BIT ((STATUS) and 07Fh), 2 es 0 se vuelve a verificar
    RETURN
    ...
    BCF STATUS, 2 ;Se limpia el BIT ((STATUS) and 07Fh), 2
    RETURN
